        ...
        if os.path.isdir(dir_file + '\\users\\' + str(user_id)):
            ...
            if len(message.text.split()) > 1:    
                ...
                if os.path.isfile(dir_file + '\\ref\\' + message.text.split()[1] + '.txt'):
                    bot.send_message(message.chat.id, 'Вас пригласил пользователь ' + message.text.split()[1])
                    os.chdir(dir_file + '\\users\\' + str(user_id))
...
def main_page(message):
    ...
    bot.send_message(message.chat.id, text='Приветствую ещё раз', reply_markup=keyboard)
bot.polling()
